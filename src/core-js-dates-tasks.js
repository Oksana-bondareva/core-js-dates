...
function getTime(date) {
  const dateGive = new Date(date);
  let hour = dateGive.getHours();
  let minute = dateGive.getMinutes();
  let second = dateGive.getSeconds();
  if (hour < 10) {
    hour = `0${hour}`;
  }
  if (minute < 10) {
...
function getNextFriday(date) {
  const dateGive = new Date(date);
  const dateNew = new Date(dateGive.getTime());
  return new Date(
...
function formatDate(date) {
  ...
  return dates.toLocaleString('en', { timeZone: 'UTC' });
}
...
function getWeekNumberByDate(date) {
  ...
  const oneJanuary = new Date(currentDate.getUTCFullYear(), 0, 1);
  const day = 24 * 60 * 60 * 1000;
  const numberOfDays = (currentDate.getTime() - oneJanuary.getTime()) / day;
  const result = Math.ceil((oneJanuary.getDay() + 1 + numberOfDays) / 7);
  return result;
}
...
function getQuarter(date) {
  const dateGive = new Date(date);
  const month = dateGive.getMonth() + 1;
